...
$\mathbf{8.54\times10^{6}~\mu\text{m}\cdot\mu\text{W}\cdot\text{ps}^{2}}$
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{../figures/power_320ps.png}
  \caption{Instantaneous and time-averaged supply power at $T=320$\,ps
    during the FOM workload. The full simulation spans 128 clock cycles
    ($16\ \text{addresses}\times 2\ \text{patterns}\times 4\ \text{repeats}
    = 128\ \text{cycles}\times 320\,\text{ps}\approx 41\,$ns),
    consisting of four back-to-back repetitions of (write all-0s to every
    address, write all-1s to every address). The red dashed line is the
    arithmetic mean over the entire run including the initial transient:
    $P=54.74\uW$.}
  \label{fig:power}
\end{figure}
...
the period sweep grid. The workload is constructed exactly as the handout
specifies -- four repetitions of (write all-0s, write all-1s) -- and
requires one cycle per address per pattern, for a total of
$16\times 2\times 4 = 128$ cycles ($\approx 41\,$ns). The average is taken
over the entire run (including the initial settling transient so that no
data is discarded), giving
\[
  P = 54.74\uW.
\]
...
\begin{align*}
  \mathrm{FOM}
  &= 60 \cdot 0.308\um \cdot 54.74\uW \cdot (91.9\,\text{ps})^2 \\
  &= 60 \cdot 0.308 \cdot 54.74 \cdot 8445.61
     \;\mu\text{m}\cdot\mu\text{W}\cdot\text{ps}^{2} \\
  &\approx \mathbf{8.54\times10^{6}\ \mu\text{m}\cdot\mu\text{W}\cdot\text{ps}^{2}}.
\end{align*}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lr@{}}
    ...
    Average power $P$                 & $54.74\uW$ \\
    \textbf{FOM}                      & $\mathbf{8.54\times10^{6}\ \mu\text{m}\cdot\mu\text{W}\cdot\text{ps}^{2}}$ \\
    \bottomrule
  \end{tabular}
\end{table}
